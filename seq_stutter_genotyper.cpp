...
void SeqStutterGenotyper::get_alleles(std::string& chrom_seq, std::vector<std::string>& alleles){
  ...
  std::string left_flank, right_flank;
  int left_trim, right_trim;
  if (start >= region_->start()){
    left_flank = uppercase(chrom_seq.substr(region_->start(), start-region_->start()));
    left_trim  = 0;
  }
  else {
    left_flank = "";
    left_trim  = region_->start()-start;
  }

  if (end <= region_->stop()){
    right_flank = uppercase(chrom_seq.substr(end, region_->stop()-end));
    right_trim  = 0;
  }
  else {
    right_flank = "";
    right_trim  = end-region_->stop();
  }
