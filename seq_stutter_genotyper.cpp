...
void SeqStutterGenotyper::init(std::vector< std::vector<BamTools::BamAlignment> >& alignments, 
  ...
  if (ref_vcf_ != NULL){
    ...
    bool success;
    log_allele_priors_ = extract_vcf_alleles_and_log_priors(ref_vcf_, region_, sample_indices_, alleles_, pos_, success);
    num_alleles_       = alleles_.size();
    if (success){
      assert(num_alleles_ > 0);

      // Construct the haplotype using the set of alleles
