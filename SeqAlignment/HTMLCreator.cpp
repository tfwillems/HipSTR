...
void writeReferenceString(std::string& reference_string, 
  ...
  for (int i = 0; i < reference_string.size(); i++){
	...
	output << "<td class=\"" << (char)toupper(reference_string[i]) << "reftd\">" << reference_string[i] << "</td>";
    }
  }
  output
    << "\t\t</tr>" << "\n"
...
void writeAlignmentStrings(std::string& reference_string, 
  ...
  if (vcf_data != NULL){
    ...
    bool set_region = vcf_data->setRegion(locus_id);
    if (!set_region) printErrorAndDie("Failed to jump to VCF region");
    bool read_variant = vcf_data->getNextVariant(variant);
    if (!read_variant || variant.position != 0) printErrorAndDie("Unable to locate VCF data for locus");
  ...
  for (int i = 0; i < alignment_strings.size(); i++){
	...
	vcf_ss << info_iter->second;
      ...
      /*
      if (vcf_data != NULL){
	std::string gt  = variant.getGenotype(alignment_samples[i]);
	std::vector<std::string> gts;
	split_by_delim(gt, '/', gts);
	if (gts.size() != 2) printErrorAndDie("Improperly formatted genotype in VCF");
	int gt_a = atoi(gts[0].c_str());
	int gt_b = atoi(gts[1].c_str());
	vcf_ss << variant.alleles[gt_a].size()-variant.alleles[0].size() << "/" << variant.alleles[gt_b].size()-variant.alleles[0].size();
      }
      else
	std::cout << "VCF is NULL" << std::endl;
      */
