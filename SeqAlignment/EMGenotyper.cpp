...
/*
void getAlignmentStatuses(std::vector<BamTools::BamAlignment>& alignments, std::vector<bool>& validities){
  validities.clear();
  double MIN_FRAC = 0.05;
  double MIN_DEL  = 5;
  double MIN_INS  = 5;

  std::map<int32_t, int> del_lbound_counts;
  std::map<int32_t, int> del_rbound_counts;
  std::map<int32_t, int> ins_counts;
  int num_del = 0;
  int num_ins = 0;

  // Determine boundary counts
  for (std::vector<BamTools::BamAlignment>::iterator align_iter = alignments.begin(); align_iter != alignments.end(); align_iter++){
    int32_t position = align_iter->Position;
    std::vector<BamTools::CigarOp> cigar_ops = align_iter->CigarData;
    for(std::vector<BamTools::CigarOp>::iterator cigar_iter = cigar_ops.begin(); cigar_iter != cigar_ops.end(); cigar_iter++){
      char type  = cigar_iter->Type;
      int length = cigar_iter->Length;
      if (type == 'M')
	position += length;
      else if (type == 'I'){
	num_ins++;
	std::map<int32_t, int>::iterator pos_iter = ins_counts.find(position);
	if (pos_iter == ins_counts.end())
	  ins_counts.insert(std::pair<long,int>(position, 1));
	else
	  pos_iter->second++;
	...
	num_del++;
	std::map<int32_t, int>::iterator l_iter = del_lbound_counts.find(position);
	std::map<int32_t, int>::iterator r_iter = del_rbound_counts.find(position+length-1);
	if(l_iter == del_lbound_counts.end())
	  del_lbound_counts.insert(std::pair<long,int>(position, 1));
	else
	  l_iter->second++;

	if(r_iter == del_rbound_counts.end())
	  del_rbound_counts.insert(std::pair<long,int>(position+length-1, 1));
	else
	  r_iter->second++;
  ...
  // Determine if each alignment is valid based on its indel bounds
  int min_for_del = std::max(num_del*MIN_FRAC, MIN_DEL);
  int min_for_ins = std::max(num_ins*MIN_FRAC, MIN_INS);
  int num_filtered = 0;
  for (std::vector<BamTools::BamAlignment>::iterator align_iter = alignments.begin(); align_iter != alignments.end(); align_iter++){
    int32_t position = align_iter->Position;
    std::vector<BamTools::CigarOp> cigar_ops = align_iter->CigarData;
	...
	std::map<int32_t, int>::iterator pos_iter = ins_counts.find(position);
	if (pos_iter->second < min_for_ins){
	  is_valid = false;
	  break;
	}
      }
	...
	std::map<int32_t, int>::iterator l_iter = del_lbound_counts.find(position);
	std::map<int32_t, int>::iterator r_iter = del_rbound_counts.find(position+length-1);
	if (l_iter->second < min_for_del && r_iter->second < min_for_del){
	  is_valid = false;
	  break;
	}
	position += length;
  ...
  std::cerr << "Filtered " << num_filtered << " out of " << alignments.size() << " alignments due to improbable indel bounds" << std::endl;
}
*/
