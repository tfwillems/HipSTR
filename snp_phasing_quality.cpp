...
void add_log_phasing_probs(BamTools::BamAlignment& aln, SNPTree* tree, BaseQuality& base_qualities, double& log_p1, double& log_p2){
  std::vector<SNP> snps;  
  tree->findContained(aln.Position, aln.GetEndPosition(), snps);
  if (snps.size() != 0){
...
void calc_het_snp_factors(std::vector<BamTools::BamAlignment>& str_reads, std::vector<BamTools::BamAlignment>& mate_reads, 
  ...
  for (unsigned int i = 0; i < str_reads.size(); i++){
    ...
    add_log_phasing_probs(str_reads[i],  snp_tree, base_qualities, log_p1, log_p2);
    add_log_phasing_probs(mate_reads[i], snp_tree, base_qualities, log_p1, log_p2);
