...
class HapBlock {
 ...
 public:
  HapBlock(int32_t start, int32_t end, const std::string& ref_seq, bool vcf_based){
    start_     = start;
    end_       = end;
    vcf_based_ = vcf_based;
    min_size_  = (int)ref_seq.size();
    max_size_  = (int)ref_seq.size();
    seqs_.push_back(ref_seq);
    removable_.push_back(false);
  ...
  bool fromVCF() const { return vcf_based_; }

  virtual void add_alternate(const std::string& alt, bool removable){
    seqs_.push_back(alt);
    removable_.push_back(removable);
  ...
  bool removable(unsigned int index) const {
    if (index < seqs_.size())
      return removable_[index];
    else
      throw std::out_of_range("Index out of bounds in HapBlock::removable()");
  }
  ...
  virtual HapBlock* reverse(){
    ...
    HapBlock* rev_block = new HapBlock(end_-1, start_-1, rev_ref_seq, vcf_based_);
  ...
  virtual HapBlock* remove_alleles(const std::vector<int>& allele_indices){
    ...
    HapBlock* new_block = new HapBlock(start_, end_, seqs_[0], vcf_based_);
    for (unsigned int i = 1; i < seqs_.size(); ++i){
