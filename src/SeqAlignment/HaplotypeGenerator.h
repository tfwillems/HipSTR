...
class HaplotypeGenerator {
 private:
  ...
  int32_t LEFT_PAD, RIGHT_PAD; // When extracting alleles from reads, we pad by these amounts to improve the capture of proximal indels
  ...
  void select_candidate_alleles(const std::string& ref_seq, int ideal_min_length,
 ...
 public:
  ...
  bool add_vcf_nonrepeat_haplotype_block(int32_t pos, const std::string& chrom_seq,
  ...
  bool add_repeat_haplotype_block(const Region& region, const std::string& chrom_seq, const std::vector< std::vector<Alignment> >& alignments,
				  const StutterModel* stutter_model);
