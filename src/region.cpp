...
void readRegions(const std::string& input_file, uint32_t max_regions, const std::string& chrom_limit,
		 std::vector<Region>& regions, std::ostream& logger){
  ...
  while (std::getline(input, line) && regions.size() < max_regions){
    ...
    if (!(iss >> chrom >> start >> stop >> period >> ref_copy))
      printErrorAndDie("Error in region file. \nRequired format is tab-delimited columns CHROM START STOP PERIOD NCOPIES\n Bad line: " + line);
    if (start < 1)      printErrorAndDie("Error in region file. \n Region has a START < 1, but START must be >= 1\n Bad line: " + line);
    if (stop <= start)  printErrorAndDie("Error in region file. \n Region has a STOP <= START. Bad line: " + line);
    if (period < 1)     printErrorAndDie("Error in region file. \n Region has a PERIOD < 1. Bad line: " + line);
    if (period > 9)     printErrorAndDie("Error in region file. \n Region has a PERIOD > 9. Bad line: " + line);
