...
do
    for phase_freq in 0.75 #0.01 0.25
    do	
	# Estimate the underlying stutter model
	res=`./em_stutter_test garb$count.str_reads.txt $haploid 4 $phase_freq | cut -f 4-`
	echo $mu $beta $p_geom $stutter_geom $stutter_down $stutter_up $haploid $nsamp $read_counts $phase_freq  $res >> stutter_results_5.txt
    done
done
